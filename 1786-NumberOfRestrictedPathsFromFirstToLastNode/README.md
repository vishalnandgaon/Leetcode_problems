...
A path from node `start` to node `end` is a sequence of nodes `[z_0, z_1,z_2, ..., z_k]` such that `z_0= start` and `z_k = end` and there is an edge between `z_i` and `z_i+1` where `0 <= i <= k-1`.
...
![image](https://assets.leetcode.com/uploads/2021/02/17/restricted_paths_ex22.png)
